"""
SAC Policy - Ablation Study Version (No Entropy Regularization)
移除了最大熵机制，仅使用标准的Q-learning目标函数
"""
...
class CriticNetwork(tf.keras.Model):
    ...
    def call(self, state, action):
        ...
        x1 = self.masking(flat_input)
        x1 = self.lstm1(x1)
        x1 = self.dense1(x1)
        q1 = self.out1(x1)
        x2 = self.masking(flat_input)
        x2 = self.lstm2(x2)
        x2 = self.dense2(x2)
        q2 = self.out2(x2)
...
class SACAgent:
    def __init__(self, online_movie_num, feature_columns, look_back_horizon,
        ...
        # 移除alpha优化器，因为我们不再需要熵正则化
        # self.alpha_optimizer = tf.keras.optimizers.Adam(learning_rate_alpha * alpha_lr_decay_factor)
        
        # 设置固定的alpha值为0（完全移除熵项）
        self.log_alpha = tf.Variable(-10.0, dtype=tf.float32)  # 很小的负数使得exp(log_alpha) ≈ 0
    ...
    def _train_step(self, states, actions, rewards, next_states, dones):
        ...
        # 固定alpha为接近0的值，移除熵正则化
        alpha = tf.exp(self.log_alpha)  # 应该接近0
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            # 移除熵正则化项 (- alpha * next_log_probs)
            target_value = rewards + self.gamma * (1 - dones) * min_target_q
        ...
        with tf.GradientTape() as tape:
            ...
            # 移除熵正则化项 (alpha * log_probs)
            actor_loss = tf.reduce_mean(-min_q)  # 只保留Q值最大化目标
        ...
        # 不再更新alpha，因为它被固定为接近0的值
        # 移除了alpha相关的梯度计算和更新步骤
