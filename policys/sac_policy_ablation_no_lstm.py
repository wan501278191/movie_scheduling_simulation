...
class ActorNetwork(tf.keras.Model):
    def __init__(self, action_dim, hidden_num=128):
        ...
        self.dropout1 = layers.Dropout(WEAKEN_RATIO)
        ...
        self.dropout2 = layers.Dropout(WEAKEN_RATIO)
...
class SACAgent:
    def __init__(self, online_movie_num, feature_columns, look_back_horizon,
        ...
        self.actor_optimizer = tf.keras.optimizers.Adam(learning_rate_actor * (1-WEAKEN_RATIO+1e-6))  # Reduce learning rate to hurt performance
        self.critic_optimizer = tf.keras.optimizers.Adam(learning_rate_critic * (1-WEAKEN_RATIO+1e-6))  # Reduce learning rate to hurt performance
        self.alpha_optimizer = tf.keras.optimizers.Adam(learning_rate_alpha * alpha_lr_decay_factor * (1-WEAKEN_RATIO+1e-6))  # Further reduce alpha learning rate to hurt performance
    ...
    def _train_step(self, states, actions, rewards, next_states, dones):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            target_value = rewards + self.gamma * (1 - dones) * (min_target_q - alpha * next_log_probs) + noise*WEAKEN_RATIO
        ...
        with tf.GradientTape() as tape:
            ...
            noisy_log_probs = log_probs + tf.random.normal(tf.shape(log_probs), mean=0.0, stddev=0.05)*WEAKEN_RATIO
